\subsection{iOS}
	...
	\subsubsection{Schutz auf Dateiebene}\label{sec:filesecurity}
		...
		\begin{itemize}
		  \item Complete Protection: Klassenschlüssel wird mit Schlüssel, abgeleitet
		  aus passcode und UID, verschlüsselt. 10 Sekunden (wenn das sofortige
		  "`Passwort benötigt"' Funktion eingestellt wurde) nach Sperren des Gerätes
		  wird dieser Schlüssel verworfen.
		  \item Protected Unless Open: Während das Gerät gesperrt ist, können
		  bestimmte Dateien, wie ein E-Mail Anhang, der heruntergeladen wird, dennoch
		  geschrieben werden. Hier werden asymmetrische elliptische Kurven eingesetzt
		  (ECDH).
		  \item Protected Until First User Authentication: Der einzige unterscheid zu
		  \textsl{Complete Protection} besteht darin, dass der genutzte Schlüssel beim
		  Sperren des Gerätes nicht verworfen wird. Diese Klasse wird per Standard für
		  Drittanbieter Apps verwendet, wenn nicht explizit anders angewiesen.
		  \item No Protection: Hier wird der Klassenschlüssel nur mit der UID
